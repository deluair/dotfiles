---
name: Session start/exit dotfiles workflow
description: When user says "start" run dotfiles pull, when user says "exit" run dotfiles push. No cheatsheet needed, just do it.
type: feedback
---
...
**When user says "exit" (ending session):**
```bash
...
**How to apply:** Listen for "start" or "exit" as session bookend signals in ANY project, on ANY machine. This is a global instruction, not project-specific. Run the appropriate make command immediately without asking. Also run `make -C ~/dotfiles backup` if user says "backup" or it's been a while.
