...
- **Protect VPS trade.db**: See [feedback_protect_tradedb.md](feedback_protect_tradedb.md). VPS trade.db is immutable (`chattr +i`). Never touch without explicit permission.
- **No side branches**: See [feedback_no_side_branches.md](feedback_no_side_branches.md). All work on main. No feature branches unless explicitly asked.
- **No OMTT label**: See [feedback_no_omtt_label.md](feedback_no_omtt_label.md). Always "BDPolicyLab", never "OMTT".
- **Clean WAL/SHM**: See [feedback_clean_wal_shm.md](feedback_clean_wal_shm.md). Remove stale WAL/SHM during deploys, backups, transfers.

## VPS References

- **Full VPS structure**: See [reference_vps_structure.md](reference_vps_structure.md). All services, DB files, directories, symlinks, disk usage.
- **TradeWeave VPS layout**: See [reference_tradedb_vps_layout.md](reference_tradedb_vps_layout.md). PM2 cwd, symlinks, DB paths, rclone/Google Drive backup.
...
|---------|----------------|-------------------|-------|
| **BDPolicyLab** | `data/bdpolicy.db` (41MB) | `db_backups/bdpolicylab_bdpolicy_latest.db` | Also timestamped copies |
| **BDPolicyLab** | `data/bangladesh.db` (43MB) | `db_backups/bdpolicylab_bangladesh_latest.db` | Copy of BDFacts DB for BDPolicyLab use |
| **BDPolicyLab** | `data/trade/` (3.9GB) | `bdpolicylab_trade_data/trade/` | CSV, parquet, xlsx research data |
| **BDPolicyLab** | `data/baci.db` (215MB) | `db_backups/bdpolicylab_baci_latest.db` | BD trade flows 1995-2024, HS92, 1.9M rows |
| **BDPolicyLab** | `data/` (raw source files) | `bdpolicylab_raw_data/data/` | CSVs, geo, hdx, fao, bbs, etc. (was `bd_policy/`) |
| **BACI** | All 7 HS revision zips + xlsx | `trade_backup/baci_zips/` | HS92/96/02/07/12/17/22 + HSCodeandDescription.xlsx |
| **BDPolicyLab** | `data/trade_flagship_results.json` | `bdpolicylab_trade_data/trade_flagship_results.json` | |
...
| **TradeWeave** | `trade.db` (19GB) | Google Drive root (`dulal1986@gmail.com`) | Primary backup. Also: VPS is immutable (`chattr +i`) |
...
| **BDPolicyLab** | `$GITHUB_USER/bdpolicylab` (`~/bdpolicylab`) | bdpolicylab.com | Future business: pure think tank, consultancy | Policy-first, AI-augmented |
...
- BDPolicyLab (future business) -> values 1, 2, 3: pure think tank for BD, consultancy/business revenue
